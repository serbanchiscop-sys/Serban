...
export function Family() {
  ...
  // Storage is unlimited & free for everyone — show usage with an Unlimited cap.
  const [quota, setQuota] = useState<Quota>({ usedGb: 4.8, totalGb: 'unlimited' });
  useEffect(() => { void getQuota().then(setQuota); }, []);
  const storageLabel = `${quota.usedGb} GB · Unlimited`;
    ...
    <div style={{ padding: '6px 18px 26px' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #EDF0F4', borderRadius: 18, padding: 16, boxShadow: 'var(--shadow-sm)', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 12.5, color: '#1F8A5B', fontWeight: 700 }}>{storageLabel}</div>
        </div>
        <div style={{ height: 8, borderRadius: 999, background: '#EEF1F6', overflow: 'hidden' }}>
          <div style={{ width: '14%', height: '100%', background: 'linear-gradient(90deg,#4CA8E4,#1B4794)' }} />
        </div>
        <div style={{ fontSize: 12.5, color: '#6B7280', marginTop: 9 }}>
          Every photo &amp; video backed up safely — <strong style={{ color: '#1F8A5B' }}>free, forever</strong>.
        </div>
      </div>
      ...
      {!state.premium && (
        <button onClick={() => open('paywall')} style={{ width: '100%', position: 'relative', border: 'none', cursor: 'pointer', borderRadius: 18, overflow: 'hidden', textAlign: 'left', padding: 18, background: 'linear-gradient(140deg,#FF9A6B,#F2613F)', color: '#fff', boxShadow: '0 14px 30px -12px rgba(242,97,63,.6)' }}>
          ...
          <div style={{ fontSize: 19, fontWeight: 800, marginTop: 6, letterSpacing: '-.01em' }}>Original quality &amp; longer videos</div>
          <div style={{ fontSize: 13, opacity: .92, marginTop: 4 }}>From €2.50 / month · 7-day free trial</div>
