 ...
 * with real Claude-generated text when a backend is configured. */
...
const STATIC_STORY = {
  title: 'The day Mila walked',
  paragraphs: [
    'It was a grey Tuesday afternoon when Mila let go of the sofa. One wobbly step, then two — arms out like a tiny tightrope walker, eyes fixed on Roan’s outstretched hands.',
    'By the third step the whole room was cheering. She sat down with a triumphant thud and looked up, as if to say: did you all see that? We did, Mila. We always will.',
  ],
};
...
export function Story() {
  ...
  // Real generation only when a backend is configured, so the offline demo
  // stays pixel-identical.
  const [story, setStory] = useState(STATIC_STORY);
  useEffect(() => {
    if (!isSupabaseConfigured) return;
    let alive = true;
    void generateStory('First steps', 'Mila').then((s) => { if (alive && s.paragraphs.length) setStory(s); });
    return () => { alive = false; };
  }, []);
